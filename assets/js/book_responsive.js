...
function goToSlide(slideNo) {
    ...
    order_counter = 1;
    if(global_audo_muted && !global_senhas){
        console.log("audio and senias off");
        $("#play-pause").fadeTo("fast", 0.3);
    }else{
        $("#play-pause").fadeTo("fast", 1);
    }
    if(animation_ongoing) return;
    let slide_to_go = slides_dictionary[slideNo];
    if(slide_to_go.skip_simple_on && global_simple){
        console.log("skip slide on simpe");
    ...
    if (slideNo >= global_minSlide && slideNo <= (slides_dictionary.length - 1)) {
        ...
        if(slide_to_go.is_multiple_select){
            ...
            $("#" + slide_to_go.slide_name).find('.target_hide').each(function () {
                $(this).show();
            })
            $("#" + slide_to_go.slide_name).find('.target_text').each(function () {
                $(this).html('');
            })
        }
    ...
    switch (value) {
        case 1: // correct
            if(order == 0 || order == order_counter){
                ...
                audioAnswer = new Audio("assets/audio/reutilizar/respuesta_correcta.m4a");
                ...
                if(audioAnswer) audioAnswer.pause();
                // audioAnswer = new Audio("assets/audio/audioprueba/r_orden_incorrecto.m4a");
                // todo cambiar por audio de orden incorrecto
                audioAnswer = new Audio("assets/audio/reutilizar/ord_inco_vuelve_intentar.m4a");
        ...
        default:
            if(audioAnswer) audioAnswer.pause();
            audioAnswer = new Audio("assets/audio/reutilizar/vuelve_a_intentar.m4a");
            if(!global_audo_muted) audioAnswer.play();
...
function insertText(indicator_id, insert_text, target, hide_target){
    ...
    $("#" + target).append("<div>" + insert_text + "</div>");
}
    ...
    if (audio_element) {
        ...
        item.addClass('read-selected');
         ...
         if (videoUrl && UrlExists(videoUrl)) {
            ...
            //  console.log("-------> entro en el else del video no encontrado");
             $(video).find('source').attr('src', "");
             video.load();
            //  console.log("----->audio_element",audio_element);
            //  console.log("----->audio_element.paused",audio_element.paused);
             if (audio_element.paused) {
                 ...
                 audio_element = false;
                 console.log("playNextAudio video doesnt exist");
...
function videoZoom() {
    if(video_size==1){
        video_size=2;
        $('#content-column').removeClass('col-9').addClass('col-8');
        $('#video-column').removeClass('col-3').addClass('col-4');
    }else if(video_size==2){
        video_size=3
        $('#content-column').removeClass('col-8').addClass('col-7');
        $('#video-column').removeClass('col-4').addClass('col-5');
    }
}
...
function videoZoomOut() {
    if(video_size==2){
        video_size=1;
        $('#content-column').removeClass('col-8').addClass('col-9');
        $('#video-column').removeClass('col-4').addClass('col-3');
    }else if(video_size==3){
        video_size=2
        $('#content-column').removeClass('col-7').addClass('col-8');
        $('#video-column').removeClass('col-5').addClass('col-4');
    }
}
